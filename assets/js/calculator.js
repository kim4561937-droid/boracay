    ...
    determineAssetStatus(startingAsset, finalAsset, projection) {
      ...
      if (hasDepleted || finalAsset <= 0 || finalAsset < startingAsset * 0.9) {
        return {
          status: 'depletion',
          name: '자산 소진형',
          description: '현재 가정에서는 경제적 자유 달성 후 원금을 점진적으로 소진하게 됩니다. 기대수명 전에 자산이 고갈될 위험이 있으므로 월 저축액을 늘리거나, 은퇴 후 수익률을 높이거나, 목표 생활비를 줄이는 조정을 권장합니다.'
        };
      }
      ...
      if (finalAsset >= startingAsset * 0.9 && finalAsset <= startingAsset * 1.1) {
        return {
          status: 'maintenance',
          name: '자산 유지형',
          description: '현재 가정에서는 경제적 자유 달성 후 원금을 크게 훼손하지 않고 은퇴 생활을 안정적으로 유지할 수 있습니다. 자산에서 발생하는 연간 수익이 연 생활비 지출과 조화를 이루는 견고한 은퇴 설계입니다.'
        };
      }
